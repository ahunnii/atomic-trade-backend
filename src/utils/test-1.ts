...
export function getApplicableDiscounts(
  ...
  return discounts.filter(
    (d) =>
      isDiscountActive(d, now) &&
      isDiscountValidForCustomer(d, customerId) &&
      isDiscountWithinUsageLimits(d) &&
      meetsMinimums(d, cartItems),
  );
}
...
export function applyBestProductDiscounts(
  ...
  return cartItems.map((item) => {
    const productDiscounts = discounts.filter(
      (d) =>
        d.type === "PRODUCT" && discountAppliesToVariant(d, item.variantId, []),
    );
...
export function calculateCartDiscounts({
  ...
  const updatedCartItems = applyBestProductDiscounts(cartItems, validDiscounts);
