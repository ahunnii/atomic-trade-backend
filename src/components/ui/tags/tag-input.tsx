...
import { Autocomplete } from "~/components/ui/tags/auto-complete";
...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>(
  (props, ref) => {
    ...
    if (
      ...
      toastService.error("maxTags and minTags cannot be less than 0");
      return null;
    }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (
        ...
        if (minLength && newTagText.length < minLength) {
          ...
          toastService.error("Tag is too short");
          return;
        }

        // Validate maxLength
        if (maxLength && newTagText.length > maxLength) {
          toastService.error("Tag is too long");
    ...
    return (
      ...
      >
        ...
        {showCount && maxTags && (
          <div className="flex">
            <span className="text-muted-foreground mt-1 ml-auto text-sm">
              {`${tagCount}`}/{`${maxTags}`}
            </span>
          </div>
        )}
