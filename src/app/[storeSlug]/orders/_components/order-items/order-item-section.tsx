...
import { DiscountDialog } from "../payments/discount-dialog";
import { CustomOrderItemDialog } from "./custom-order-item-dialog";
...
export const OrderItemSection = ({ form, loading, products }: Props) => {
  ...
  return (
    <Card className="space-y-6 px-6">
      ...
      <ProductSelectDialog
        ...
        //   // Get current items and filter out any that match the new items' variantIds
        //   const currentItems = form.getValues("orderItems") ?? [];
        //   const filteredItems = currentItems.filter(
        //     (item) =>
        //       !newItems.some((newItem) => newItem.variantId === item.variantId),
        //   );

        //   // Combine filtered items with new items
        //   form.setValue("orderItems", [...filteredItems, ...newItems]);
        // }}
      />
    </Card>
  );
